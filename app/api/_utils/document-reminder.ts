...
export async function sendDocumentReminder(
  ...
  const { data: req, error: reqError } = await admin
    ...
    .select(`
      id, project_id, activity_id, name, description, status, visibility, deadline_at, deleted_at,
      project:project_id(id, title, client:profiles!projects_client_id_fkey(full_name, email)),
      activity:activity_id(id, visibility, phase:phase_id(id, visibility))
    `)
    .eq('id', requestId)
    .is('deleted_at', null)
    .maybeSingle()
  ...
  // Reminder-ul trimite clientul spre platformă — dacă cererea (sau activitatea/faza
  // părinte) e încă draft, clientul n-o vede acolo și emailul îl trimite degeaba.
  if (!isClientVisibleDocument(req)) {
    return {
      ok: false,
      status: 409,
      error: 'Cererea nu este publicată — public-o înainte să trimiți reminder clientului.',
    }
  }

  // Fără termen limită nu există „urgență" de comunicat: orice tip am alege, textul
  // emailului ar promite clientului un termen care nu există.
  const reminderType = getReminderType((req as any).deadline_at)
  if (!reminderType) {
    return {
      ok: false,
      status: 400,
      error: 'Cererea nu are termen limită — setează unul înainte să trimiți reminder.',
    }
  }
