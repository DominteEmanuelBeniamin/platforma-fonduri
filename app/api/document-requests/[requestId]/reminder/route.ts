...
export async function POST(
  ...
  try {
    ...
    // Obține cererea + project_id
    const { data: req, error: reqError } = await admin
      .from('document_requirements')
      .select('id, project_id, reminder_sent_at')
      .eq('id', requestId)
      .maybeSingle()
    ...
    // Toggle
    const newValue = req.reminder_sent_at ? null : new Date().toISOString()

    const { error: updateError } = await admin
      .from('document_requirements')
      .update({ reminder_sent_at: newValue })
      .eq('id', requestId)
    ...
    return NextResponse.json({ reminder_sent_at: newValue })
  } catch (e: any) {
