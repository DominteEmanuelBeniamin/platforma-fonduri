...
const COLUMNS: { key: ProjectColumnKey; label: string; numeric?: boolean; hint?: string }[] = [
  ...
  { key: 'waiting', label: 'De rezolvat', hint: 'Munca rămasă, împărțită după cine o mișcă mai departe: echipa (activități neterminate, documente de verificat) sau clientul (documente cerute sau respinse)' },
  { key: 'deadline', label: 'Următorul termen' },
  { key: 'week', label: 'Săptămâna asta', numeric: true, hint: 'Termene în următoarele 7 zile; depășitele au coloana lor' },
...
function ProjectDashboardContent() {
  ...
  return (
    ...
    <div className="project-scope space-y-4">
      ...
      {/* Rezumatul și controalele apar doar peste un tabel adevărat: altfel ar
          sta un „0 proiecte" deasupra unei erori sau a unui spinner. */}
      {!loading && !error && (
          ...
          {showSearch && (
            <label className="relative">
              ...
              <input
                ...
                className="w-56 rounded-lg border border-[var(--p-border)] bg-[var(--p-surface)] py-1.5 pl-8 pr-2.5 text-xs text-[var(--p-ink)] placeholder:text-[var(--p-ink-faint)] focus:border-[var(--p-accent)] focus:outline-none focus:ring-1 focus:ring-[var(--p-accent)]"
              />
            </label>
          )}
          ...
          <EmptyState
            ...
            action={
              <button
                ...
                className="text-sm font-semibold text-[var(--p-accent)] underline underline-offset-2"
              >
                Șterge căutarea
              </button>
            }
...
/**
 * Cifrele de deasupra tabelului, într-o singură propoziție. Nu carduri mari:
 * Home are deja indicatorii lui, iar două seturi ar fi început să se contrazică.
 */
function SummaryLine({ summary }: { summary: DashboardSummary }) {
  const pieces: { text: string; tone?: 'danger' | 'accent' }[] = [
    { text: countLabel(summary.projects, 'proiect', 'proiecte') },
    summary.overdue > 0
      ? {
          text: `${countLabel(summary.overdue, 'termen depășit', 'termene depășite')} în ${countLabel(
  ...
  if (summary.dueSoon > 0) {
    pieces.push({ text: `${countLabel(summary.dueSoon, 'termen', 'termene')} săptămâna asta` })
  }
  if (summary.waitingUs > 0) {
    pieces.push({ text: `${countLabel(summary.waitingUs, 'element', 'elemente')} ${WAITING_LABELS.us}`, tone: 'accent' })
  }
    ...
    <p className="text-sm text-[var(--p-ink-soft)]">
      {pieces.map((piece, index) => (
        <Fragment key={piece.text}>
          {index > 0 && <span className="mx-1.5 text-[var(--p-ink-faint)]">·</span>}
          <span
            className={
              piece.tone === 'danger'
                ? 'font-semibold text-[var(--p-danger)]'
                : piece.tone === 'accent'
                ? 'font-semibold text-[var(--p-accent)]'
                : index === 0
...
/** „de o zi", „de 12 zile", „de 21 de zile" — vechimea de lângă numărul roșu. */
...
function ProjectRow({
  ...
  return (
    <Fragment>
      ...
      <tr onClick={onToggle} className="cursor-pointer transition-colors hover:bg-[var(--p-surface-2)]">
        <td className="px-4 py-3">
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-md text-[var(--p-ink-faint)] transition-colors hover:bg-[var(--p-surface-2)] hover:text-[var(--p-ink)] focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--p-accent)]"
            >
            ...
            {!row.active && (
              <span className="rounded-full bg-[var(--p-draft-soft)] px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-[var(--p-draft)]">
                Încheiat
              </span>
            )}
          </div>
        ...
        <td className="px-4 py-3 text-[var(--p-ink-soft)]">
          {row.client_name ?? <span className={faint}>Fără client</span>}
        </td>
        ...
        <td className="px-4 py-3">
          {row.waiting_us + row.waiting_client === 0 ? (
            ...
            <span className="flex flex-wrap items-center gap-1">
              {row.waiting_us > 0 && (
                <span className="rounded-md bg-[var(--p-accent-soft)] px-1.5 py-0.5 text-[11px] font-bold text-[var(--p-accent)]">
                  {row.waiting_us} {WAITING_LABELS.us}
                </span>
              )}
              {row.waiting_client > 0 && (
                <span className="rounded-md bg-[var(--p-surface-2)] px-1.5 py-0.5 text-[11px] font-medium text-[var(--p-ink-soft)]">
        ...
        <td className="px-4 py-3">
          {row.next_deadline === null ? (
            ...
            <span className="text-[var(--p-ink)]">
              ...
              {relative && <span className={`ml-1.5 text-xs ${faint}`}>{relative}</span>}
            </span>
          )}
        </td>
        ...
        <td className="px-4 py-3 text-right tabular-nums">
          {row.due_soon === 0 ? (
            ...
            <span className="font-semibold text-[var(--p-ink)]">{row.due_soon}</span>
          )}
        </td>
          ...
          {row.overdue === 0 ? (
            ...
            <span>
              ...
              {row.oldest_overdue_days !== null && (
                <span className={`block text-[11px] font-normal ${faint}`}>
                  {overdueAge(row.oldest_overdue_days)}
                </span>
              )}
            </span>
          )}
      ...
      {open && (
        <tr id={detailsId} className="bg-[var(--p-surface-2)]">
...
function ProjectDetails({ row }: { row: ProjectDashboardRow }) {
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-t border-[var(--p-border)] pt-3 text-xs text-[var(--p-ink-soft)]">
        <p>
          ...
          <span className="font-semibold text-[var(--p-ink)]">Activități</span> {row.activities.done}/
          {row.activities.total}
          <span className="mx-1.5 text-[var(--p-ink-faint)]">·</span>
          <span className="font-semibold text-[var(--p-ink)]">Documente</span> {row.requests.done}/
          {row.requests.total}
          {row.drafts > 0 && (
            <>
              <span className="mx-1.5 text-[var(--p-ink-faint)]">·</span>
              <span className="text-[var(--p-draft)]">
                {countLabel(row.drafts, 'element', 'elemente')} în pregătire, pe care clientul nu le vede
              </span>
            </>
          )}
        </p>
        ...
        <span className="flex flex-wrap items-center gap-4">
          <Link
            href={`/projects/${row.id}`}
            className="inline-flex items-center gap-1 font-semibold text-[var(--p-accent)] hover:underline"
          >
            Deschide proiectul
            <ArrowRight className="h-3 w-3" aria-hidden />
          </Link>
          <Link
            href={projectCalendarHref(row.id)}
            className="inline-flex items-center gap-1 font-semibold text-[var(--p-accent)] hover:underline"
          >
  ...
  return (
    <section aria-label={title}>
      <h3 className="flex items-baseline gap-2 px-1 pb-1.5 text-xs font-bold uppercase tracking-wide">
        ...
        {events.length > 0 && (
          <span className="text-[11px] font-medium normal-case text-[var(--p-ink-faint)]">{events.length}</span>
        )}
      </h3>

      {shown.length === 0 ? (
        <p className="rounded-xl border border-dashed border-[var(--p-border-strong)] px-3 py-4 text-center text-xs text-[var(--p-ink-faint)]">
          {empty}
        </p>
      ) : (
        <div className="overflow-hidden rounded-xl border border-[var(--p-border)] bg-[var(--p-surface)]">
          ...
          {rest > 0 && (
            <Link
              href={moreHref}
              className="flex items-center justify-center gap-1 border-t border-[var(--p-border)] bg-[var(--p-surface-2)] px-3 py-2 text-xs font-semibold text-[var(--p-accent)] hover:underline"
            >
