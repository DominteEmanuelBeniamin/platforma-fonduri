...
test('migrarea audit append-only protejează update și delete pe fiecare rând', async () => {
  const baseline = await readText('supabase/migrations/20260918080000_baseline_public_schema.sql')
  const normalized = baseline.replace(/"/g, '').replace(/\s+/g, ' ')

  assert.match(normalized, /before delete or update on public\.audit_logs/i)
  assert.match(normalized, /for each row execute function public\.prevent_audit_logs_mutation\(\)/i)
  assert.match(normalized, /raise exception[^;]+append-only/i)
})
