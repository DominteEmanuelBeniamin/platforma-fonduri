...
test('returns only the final approval while claiming both review rows', () => {
  const result = selectReviewNotificationCandidates({
    requests: [request({ status: 'approved', name: 'Oferte de pret' })],
    reviews: [
  ...
  const events = buildReviewNotificationEvents(result.candidates)
  assert.equal(events.length, 1)
  assert.equal(events[0].requestId, 'request-1')
  assert.equal(events[0].title, 'Document aprobat')
  // Numele stă separat: panoul îl pune pe primul rând, titlul rămâne acțiunea.
  assert.equal(events[0].entityLabel, 'Oferte de pret')
  assert.equal(events[0].eventKey, 'document-review:review-approved')
})
