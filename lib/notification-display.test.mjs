...
test('from yesterday down, the row shows the day and the hour', () => {
  ...
  assert.equal(formatRelativeTime(new Date('2026-08-22T18:45:00').toISOString(), now), 'sâmbătă, 18:45')
  // De la o săptămână în sus, ziua săptămânii nu mai plasează nimic.
...
test('day groups separate today, yesterday and the rest', () => {
  ...
  assert.equal(notificationDayGroup(ago(4 * DAY), now), 'Săptămâna aceasta')
