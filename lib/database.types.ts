// =====================================================
// DATABASE TYPES - ACTUALIZAT CU STATUSURI ȘI TEMPLATE-URI
// =====================================================
...
// Statusurile fazelor și activităților (rămân la fel)
...
// =====================================================
// PARTEA 2: STATUSURI GLOBALE PROIECT (NOU)
// =====================================================
...
export interface ProjectTemplate {
  ...
  measure?: { name: string; program?: { name: string } };
}
...
export interface TemplateActivity {
  ...
  created_at: string;
  // Relații
  document_requirements?: TemplateDocumentRequirement[];
}
...
// Document requirement în template
...
export interface TemplateDocumentRequirementCreate {
  template_activity_id: string;
  name: string;
  description?: string;
  is_mandatory?: boolean;
...
// =====================================================
// PARTEA 4: INTERFEȚE PRINCIPALE (ACTUALIZATE)
// =====================================================
...
// PROIECT (ACTUALIZAT - cu status_id și template_id)
export interface Project {
  id: string;
  title: string;
  client_id: string;
  status: string; // păstrăm pentru compatibilitate
  current_status_id: string | null; // NOU - referință la project_statuses
  template_id: string | null; // NOU - template-ul folosit
  progress: number;
...
export interface ProjectCreate {
  title: string;
  client_id: string;
  template_id?: string; // NOU - opțional, pentru import template
  program_id?: string;
  measure_id?: string;
  session_id?: string;
  cod_proiect?: string;
...
export interface ProjectPhase {
  id: string;
  project_id: string;
  project_status_id: string | null; // NOU - legătura la statusul mare
  name: string;
  slug: string;
  description: string | null;
  order_index: number;
...
export interface ProjectPhaseWithStatus extends ProjectPhase {
...
export interface ProjectActivityCreate {
  phase_id: string;
  name: string;
  description?: string;
  order_index?: number;
...
// =====================================================
// PARTEA 5: VIEWS ȘI TIPURI AGREGATE
// =====================================================
...
// Grupare faze pe status (pentru UI)
export interface PhasesGroupedByStatus {
  status: ProjectStatus;
  phases: ProjectPhaseWithStatus[];
...
// NOU: Helper pentru a grupa fazele pe status
export function groupPhasesByStatus(
  phases: ProjectPhaseWithStatus[],
  statuses: ProjectStatus[],
  currentStatusId: string | null
): PhasesGroupedByStatus[] {
  return statuses.map(status => {
    const statusPhases = phases.filter(p => p.project_status_id === status.id);
    const totalActivities = statusPhases.reduce((sum, p) => sum + p.total_activities, 0);
    const completedActivities = statusPhases.reduce((sum, p) => sum + p.completed_activities, 0);
    ...
    return {
      ...
      totalActivities,
      completedActivities,
      isCurrentStatus: status.id === currentStatusId
    };
  }).filter(group => group.phases.length > 0);
}

// NOU: Generare slug din text
...
// =====================================================
// PARTEA 8: SUPABASE DATABASE TYPES
// =====================================================
...
export interface Database {
  public: {
    Tables: {
      ...
      // NOU: Template-uri
      project_templates: { Row: ProjectTemplate; Insert: ProjectTemplateCreate; Update: ProjectTemplateUpdate };
      template_phases: { Row: TemplatePhase; Insert: TemplatePhaseCreate; Update: TemplatePhaseUpdate };
      template_activities: { Row: TemplateActivity; Insert: TemplateActivityCreate; Update: TemplateActivityUpdate };
      template_document_requirements: { Row: TemplateDocumentRequirement; Insert: TemplateDocumentRequirementCreate; Update: TemplateDocumentRequirementUpdate };
