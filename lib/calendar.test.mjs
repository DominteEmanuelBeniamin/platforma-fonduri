...
test('munca fără responsabil stă mereu prima, oricum s-ar sorta', () => {
  const rows = [
  ...
  // Iese din clasament, nu se ia la întrecere cu oamenii: e prima problemă de
  // rezolvat, nu cel mai încărcat consultant.
  assert.deepEqual(sortConsultantRows(rows, 'urgency', 'asc').map(r => r.id), ['nimeni', 'ana', 'ion'])
  assert.deepEqual(sortConsultantRows(rows, 'overdue', 'desc').map(r => r.id), ['nimeni', 'ana', 'ion'])
  assert.deepEqual(sortConsultantRows(rows, 'overdue', 'asc').map(r => r.id), ['nimeni', 'ion', 'ana'])
  assert.deepEqual(sortConsultantRows(rows, 'consultant', 'asc').map(r => r.id), ['nimeni', 'ana', 'ion'])
})
