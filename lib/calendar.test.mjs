...
test('munca se strânge pe responsabilul efectiv al fiecărui element', () => {
  const rows = buildPersonDashboardRows(
    payload(
  ...
  // Doi din două proiecte: un om întins pe mai multe lucrări nu e ca unul pe una.
...
test('oamenii se strâng peste exact proiectele vizibile alături', () => {
  const full = payload(
  ...
  assert.equal(buildPersonDashboardRows(full).find(r => r.id === 'ana').total, 2)
  // Cu proiectele încheiate ascunse, cele două tabele n-au voie să descrie
  // mulțimi diferite.
  assert.equal(buildPersonDashboardRows(full, new Set(['activ'])).find(r => r.id === 'ana').total, 1)
})
...
test('cele două tabele numără la fel: totalurile lor se potrivesc', () => {
  ...
  const perPerson = summarizeRows(buildPersonDashboardRows(data))
...
test('oamenii se sortează pe aceleași reguli ca proiectele', () => {
  const rows = [
    person({ id: 'liber', label: 'Liber' }),
    person({ id: 'ars', label: 'Ars', overdue: 3, oldest_overdue_days: 40 }),
    person({ id: 'ocupat', label: 'Ocupat', overdue: 3, oldest_overdue_days: 2 }),
  ]
  assert.deepEqual(sortPersonRows(rows, 'urgency', 'asc').map(r => r.id), ['ars', 'ocupat', 'liber'])
  assert.deepEqual(sortPersonRows(rows, 'person', 'asc').map(r => r.id), ['ars', 'liber', 'ocupat'])
})

test('„proiecte" sortează pe câte lucrări îl întind pe om', () => {
  const rows = [
    person({ id: 'unul', projects: 1, total: 90 }),
    person({ id: 'cinci', projects: 5, total: 10 }),
  ]
  assert.deepEqual(sortPersonRows(rows, 'projects', 'desc').map(r => r.id), ['cinci', 'unul'])
})

test('căutarea pe oameni prinde numele, fără diacritice', () => {
  const rows = [person({ id: 'a', label: 'Ștefan Mihai' }), person({ id: 'b', label: 'Ana Pop' })]
  assert.deepEqual(filterPersonRows(rows, 'stefan').map(r => r.id), ['a'])
})

test('vederea se scrie în adresă doar când nu e cea implicită', () => {
  const params = new URLSearchParams()
  assert.equal(readDashboardView(params), 'projects')
  writeDashboardView(params, 'people')
  assert.equal(params.get('vedere'), 'oameni')
  assert.equal(readDashboardView(params), 'people')
...
test('o coloană a celuilalt tabel, rămasă în adresă, cade pe ordinea implicită', () => {
  ...
  assert.deepEqual(readPersonSort(params), { sort: 'urgency', direction: 'asc' })
...
test('sortarea pe oameni se scrie și se citește la fel ca la proiecte', () => {
  const params = new URLSearchParams()
  writePersonSort(params, nextPersonSort({ sort: 'urgency', direction: 'asc' }, 'projects'))
  assert.equal(params.get('sort'), 'projects')
  // Prima apăsare e sensul implicit al coloanei, deci nu lasă `dir` în adresă.
  assert.equal(params.get('dir'), null)
  assert.deepEqual(readPersonSort(params), { sort: 'projects', direction: 'desc' })
})

test('linkul unui om duce în calendarul lui, filtrat pe responsabil', () => {
  const url = new URL(personCalendarHref('ana', { overdueOnly: true }), 'https://x')
  ...
  const nobody = new URL(personCalendarHref(UNASSIGNED_OWNER_ID), 'https://x')
