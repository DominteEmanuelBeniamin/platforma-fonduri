...
test('proiectele fără termen viitor stau la final în ambele sensuri', () => {
  const rows = [row({ id: 'fara', label: 'A' }), row({ id: 'cu', label: 'B', next_deadline: dayOffset(5) })]
...
test('„finalizate" se sortează pe raport, nu pe numărul brut', () => {
  const rows = [
    row({ id: 'aproape', label: 'A', total: 4, done: 3 }),
    row({ id: 'mult', label: 'B', total: 40, done: 5 }),
    row({ id: 'gol', label: 'C' }),
  ]
...
test('căutarea trece peste diacritice și peste majuscule', () => {
  const rows = [
    row({ id: 'a', label: 'ACHIZIȚIE miniexcavator' }),
    row({ id: 'b', label: 'Femeia antreprenor' }),
  ]
...
test('căutarea prinde și clientul, nu doar titlul', () => {
  const rows = [
    row({ id: 'a', label: 'Proiect unu', client_name: 'TAC SRL' }),
    row({ id: 'b', label: 'Proiect doi', client_name: null }),
  ]
...
test('căutarea din două cuvinte trece peste spațiul dintre ele', () => {
  const rows = [
    row({ id: 'a', label: 'Femeia antreprenor', client_name: 'TAC SRL' }),
    row({ id: 'b', label: 'Femeia de serviciu' }),
  ]
...
// ─── Tabelul cu consultanții ───────────────────────────────────────────────────────

test('munca se strânge pe responsabilul efectiv al fiecărui element', () => {
  const rows = buildConsultantDashboardRows(
    payload(
      [project({ id: 'p1' }), project({ id: 'p2' })],
      [
        event({ id: 'a', project_id: 'p1', assignee_id: 'ana', assignee_name: 'Ana', deadline_at: dayOffset(-3) }),
        event({ id: 'b', project_id: 'p2', assignee_id: 'ana', assignee_name: 'Ana', deadline_at: dayOffset(4) }),
        event({ id: 'c', project_id: 'p1', assignee_id: 'ion', assignee_name: 'Ion' }),
      ]
    )
  )
  const ana = rows.find(r => r.id === 'ana')
  assert.equal(ana.label, 'Ana')
  assert.equal(ana.total, 2)
  assert.equal(ana.overdue, 1)
  // Doi din două proiecte: un consultant întins pe mai multe lucrări nu e ca unul pe una.
  assert.equal(ana.projects, 2)
  assert.equal(rows.find(r => r.id === 'ion').projects, 1)
})

test('munca fără nimeni în spate primește rândul ei, nu dispare', () => {
  const rows = buildConsultantDashboardRows(
    payload(
      [project()],
      [
        event({ id: 'a', assignee_id: null, assignee_name: null }),
        event({ id: 'b', assignee_id: null, assignee_name: null }),
        event({ id: 'c', assignee_id: 'ana', assignee_name: 'Ana' }),
      ]
    )
  )
  const nimeni = rows.find(r => r.id === UNASSIGNED_OWNER_ID)
  assert.equal(nimeni.assigned, false)
  assert.equal(nimeni.label, 'Fără responsabil')
  assert.equal(nimeni.total, 2)
  assert.equal(rows.find(r => r.id === 'ana').assigned, true)
})

test('consultanții se strâng peste exact proiectele vizibile alături', () => {
  const full = payload(
    [project({ id: 'activ' }), project({ id: 'incheiat', lifecycle_status: 'archived' })],
    [
      event({ id: 'a', project_id: 'activ', assignee_id: 'ana', assignee_name: 'Ana' }),
      event({ id: 'b', project_id: 'incheiat', assignee_id: 'ana', assignee_name: 'Ana' }),
    ]
  )
  assert.equal(buildConsultantDashboardRows(full).find(r => r.id === 'ana').total, 2)
  // Cu proiectele încheiate ascunse, cele două tabele n-au voie să descrie
  // mulțimi diferite.
  assert.equal(buildConsultantDashboardRows(full, new Set(['activ'])).find(r => r.id === 'ana').total, 1)
})

test('cele două tabele numără la fel: totalurile lor se potrivesc', () => {
  const data = payload(
    [project({ id: 'p1' }), project({ id: 'p2' })],
    [
      event({ id: 'a', project_id: 'p1', assignee_id: 'ana', assignee_name: 'Ana', deadline_at: dayOffset(-2) }),
      event({ id: 'b', project_id: 'p1', assignee_id: null, deadline_at: dayOffset(3) }),
      event({ id: 'c', project_id: 'p2', assignee_id: 'ion', assignee_name: 'Ion', done: true, status: 'approved' }),
      event({ id: 'd', project_id: 'p2', assignee_id: 'ana', assignee_name: 'Ana', deadline_at: null }),
    ]
  )
  const perProject = summarizeRows(buildProjectDashboardRows(data))
  const perPerson = summarizeRows(buildConsultantDashboardRows(data))
  assert.equal(perProject.overdue, perPerson.overdue)
  assert.equal(perProject.dueSoon, perPerson.dueSoon)
  assert.equal(perProject.waitingUs, perPerson.waitingUs)
})

test('consultanții se sortează pe aceleași reguli ca proiectele', () => {
  const rows = [
    consultant({ id: 'liber', label: 'Liber' }),
    consultant({ id: 'ars', label: 'Ars', overdue: 3, oldest_overdue_days: 40 }),
    consultant({ id: 'ocupat', label: 'Ocupat', overdue: 3, oldest_overdue_days: 2 }),
  ]
  assert.deepEqual(sortConsultantRows(rows, 'urgency', 'asc').map(r => r.id), ['ars', 'ocupat', 'liber'])
  assert.deepEqual(sortConsultantRows(rows, 'consultant', 'asc').map(r => r.id), ['ars', 'liber', 'ocupat'])
})

test('„proiecte" sortează pe câte lucrări îl întind pe consultant', () => {
  const rows = [
    consultant({ id: 'unul', projects: 1, total: 90 }),
    consultant({ id: 'cinci', projects: 5, total: 10 }),
  ]
  assert.deepEqual(sortConsultantRows(rows, 'projects', 'desc').map(r => r.id), ['cinci', 'unul'])
})

test('căutarea pe consultanți prinde numele, fără diacritice', () => {
  const rows = [consultant({ id: 'a', label: 'Ștefan Mihai' }), consultant({ id: 'b', label: 'Ana Pop' })]
  assert.deepEqual(filterConsultantRows(rows, 'stefan').map(r => r.id), ['a'])
})

test('vederea se scrie în adresă doar când nu e cea implicită', () => {
  const params = new URLSearchParams()
  assert.equal(readDashboardView(params), 'projects')
  writeDashboardView(params, 'consultants')
  assert.equal(params.get('vedere'), 'consultanti')
  assert.equal(readDashboardView(params), 'consultants')
  writeDashboardView(params, 'projects')
  assert.equal(params.toString(), '')
})

test('o coloană a celuilalt tabel, rămasă în adresă, cade pe ordinea implicită', () => {
  // La comutare, `sort=client` n-are corespondent între oameni: tabelul se
  // deschide în ordinea lui implicită, nu într-una inventată.
  const params = new URLSearchParams('sort=client&dir=desc')
  assert.deepEqual(readConsultantSort(params), { sort: 'urgency', direction: 'asc' })
  assert.deepEqual(readProjectSort(params), { sort: 'client', direction: 'desc' })
})

test('sortarea pe oameni se scrie și se citește la fel ca la proiecte', () => {
  const params = new URLSearchParams()
  writeConsultantSort(params, nextConsultantSort({ sort: 'urgency', direction: 'asc' }, 'projects'))
  assert.equal(params.get('sort'), 'projects')
  // Prima apăsare e sensul implicit al coloanei, deci nu lasă `dir` în adresă.
  assert.equal(params.get('dir'), null)
  assert.deepEqual(readConsultantSort(params), { sort: 'projects', direction: 'desc' })
})

test('linkul unui consultant duce în calendarul lui, filtrat pe responsabil', () => {
  const url = new URL(consultantCalendarHref('ana', { overdueOnly: true }), 'https://x')
  assert.equal(url.pathname, '/calendar')
  assert.equal(url.searchParams.get('cv'), 'list')
  assert.equal(url.searchParams.get('co'), 'ana')
  assert.equal(url.searchParams.get('cs'), 'overdue')

  // Și rândul fără nimeni duce undeva: filtrul de responsabil are „Fără responsabil".
  const nobody = new URL(consultantCalendarHref(UNASSIGNED_OWNER_ID), 'https://x')
  const filters = readFiltersFromParams(nobody.searchParams, defaultFilters('admin', 'u1'))
  assert.deepEqual(filters.owners, [UNASSIGNED_OWNER_ID])
})

test('munca fără responsabil stă mereu ultima, oricum s-ar sorta', () => {
  const rows = [
    consultant({ id: 'nimeni', label: 'Fără responsabil', assigned: false, overdue: 40, total: 197 }),
    consultant({ id: 'ana', label: 'Ana', overdue: 3 }),
    consultant({ id: 'ion', label: 'Ion', overdue: 1 }),
  ]
  // Cu 197 de elemente și 40 de depășiri ar fi ieșit prima la orice coloană și
  // ar fi împins consultanții adevărați sub ea.
  assert.deepEqual(sortConsultantRows(rows, 'urgency', 'asc').map(r => r.id), ['ana', 'ion', 'nimeni'])
  assert.deepEqual(sortConsultantRows(rows, 'overdue', 'desc').map(r => r.id), ['ana', 'ion', 'nimeni'])
  assert.deepEqual(sortConsultantRows(rows, 'overdue', 'asc').map(r => r.id), ['ion', 'ana', 'nimeni'])
  assert.deepEqual(sortConsultantRows(rows, 'consultant', 'asc').map(r => r.id), ['ana', 'ion', 'nimeni'])
})

test('numeralul din „în N zile" respectă „de" la fel ca restul textelor', () => {
  assert.equal(formatRelativeDeadline(dayOffset(3)), 'în 3 zile')
  assert.equal(formatRelativeDeadline(dayOffset(128)), 'în 128 de zile')
  assert.equal(formatRelativeDeadline(dayOffset(-20)), 'acum 20 de zile')
  assert.equal(formatRelativeDeadline(dayOffset(-13)), 'acum 13 zile')
  assert.equal(formatRelativeDeadline(dayOffset(1)), 'mâine')
  assert.equal(formatRelativeDeadline(dayOffset(0)), 'astăzi')
})
