...
test('„săptămâna asta" scoate primul proiectul cu cele mai multe termene apropiate', () => {
  const rows = [row({ id: 'calm', due_soon: 0 }), row({ id: 'aglomerat', due_soon: 6 })]
  assert.deepEqual(sortProjectRows(rows, 'week', 'desc').map(r => r.id), ['aglomerat', 'calm'])
})
