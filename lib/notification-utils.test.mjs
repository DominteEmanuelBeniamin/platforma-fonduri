...
const notificationSchema = readFileSync(
  resolve(dirname(fileURLToPath(import.meta.url)), '..', 'supabase', 'migrations', '20260918080000_baseline_public_schema.sql'),
  'utf8',
)

test('helperul SQL folosește coloana event_key din schema notifications', () => {
  const columns = notificationSchema.match(
  ...
  assert.match(notificationSchema, /on conflict \(user_id, event_key\) do nothing/)
})
...
test('cheile assignment sunt stabile, distincte la reasignare și includ versiunea', () => {
  const input = {
    projectId,
    entityType: 'activity',
    entityId,
...
test('trigger-ele de assignment folosesc cheia v2 pentru reatribuiri distincte', () => {
  assert.equal((notificationSchema.match(/txid_current\(\)/g) ?? []).length, 2)
  const keys = notificationSchema.match(/'assignment-v2:%s:%s:%s:%s:%s'/g) ?? []
  assert.equal(keys.length, 2)
  assert.equal(
    (notificationSchema.match(/coalesce\(old\.assigned_to::text, 'none'\)/g) ?? []).length,
    2,
  )
})
...
test('cursorul păstrează perechea descrescătoare created_at/id și respinge date invalide', () => {
  ...
  for (const createdAt of ['Jan 1, 2026', '2026-08-21T12:30:00.000Z,extra', '2026-08-21 12:30:00', '2026-08-21T12:30:00.000']) {
    assert.equal(
